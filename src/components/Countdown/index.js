    ...
    <div className="flex flex-col items-center justify-center h-screen bg-no-repeat bg-center bg-cover relative" style={{ backgroundImage: "url('/static/img/bg.png')" }}>
      ...
      <div className="text-center">
        <div className="text-5xl md:text-8xl text-white font-bold font-apple2mono">
          {integerDays}
          <span className="text-2xl md:text-4xl text-neutral-400 text-opacity-75 font-normal -ml-2 md:-ml-4">.{fractionalDays.slice(2)}</span>
        </div>
        <p className="text-sm md:text-2xl text-neutral-400 text-opacity-75 font-apple2mono mb-8">days till {desc || (new Date().getFullYear() + 1)}</p>
      </div>
